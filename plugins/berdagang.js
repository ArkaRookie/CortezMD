let handler = async (m, { conn, text }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastdagang > 28800000) {
    ...
    if (successChance > 0.7) { // 30% success rate
      ...
      setTimeout(() => {
        conn.reply(m.chat, `Maaf kamu dan @${who.replace(/@.+/, '')} mengalami kerugian dalam berdagang..\n\nKerugian dagang kamu sebesar -${dapat}\n${users[m.sender].money -= dapat} Money kamu\n\nKerugian dagang @${who.replace(/@.+/, '')} sebesar -${dapat}\n${users[who].money -= dapat} Money @${who.replace(/@.+/, '')}`, m, {
          contextInfo: { mentionedJid: [m.sender, who] }
        });
      }, 3600000);
    }
    ...
    m.reply(`Anda Sudah Berdagang , tunggu ${timers} lagi..`);
  }
};
